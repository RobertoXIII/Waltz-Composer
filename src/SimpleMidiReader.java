...
public class SimpleMidiReader
{
	...
	// beats per measure - this can be changed using a META message that is not handled in his class
	private int beatsPerMeasure = 3;
	...
	{
		String key = notes.get(notes.size() - 1).pitchName;
		
		if (notes.get(0).pitchName == notes.get(notes.size() - 1).pitchName)
		{
			key = notes.get(0).pitchName;
			return key;
		}
